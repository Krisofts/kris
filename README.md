...
1. Install build tools and llama.cpp:

   ```
   pkg update && pkg upgrade
   pkg install git cmake clang golang rust
...
2. Download a small GGUF coding model, e.g. Qwen2.5-Coder 1.5B or 3B
   Instruct (search "Qwen2.5-Coder-3B-Instruct-GGUF" on Hugging Face and
   grab a `Q4_K_M` quantization to keep RAM usage down on-device).

3. Start the local inference server:

   ```
   ./build/bin/llama-server -m /path/to/qwen2.5-coder-3b-instruct-q4_k_m.gguf \
       --host 127.0.0.1 --port 8080 -c 4096
   ```

4. In another Termux session, build and run KRIS:
...
5. Point KRIS at the server/model if you changed the defaults:
