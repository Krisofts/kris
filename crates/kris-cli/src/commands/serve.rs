...
impl Command for ServeCommand {
    ...
    fn execute(&self, context: &mut Context, _args: &[&str]) {
        ensure_running(&context.settings.clone());
    }
}
...
/// which call this automatically before talking to the model so the user
/// doesn't have to run `serve` by hand first. Returns whether llama-server
/// ended up reachable.
pub fn ensure_running(settings: &Settings) -> bool {
